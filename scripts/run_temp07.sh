...
PORT=8901; N=384; mkdir -p campt07
...
send_all(){ for i in $(seq 0 39); do
  $PY -c "import json;p=json.load(open('prompts40.json'))[$i][1];json.dump({'messages':[{'role':'user','content':p}],'n_predict':$N,'temperature':0.7,'top_p':0.8,'seed':42,'ignore_eos':True,'chat_template_kwargs':{'enable_thinking':False},'cache_prompt':False},open('/tmp/qt.json','w'))"
  curl -s http://127.0.0.1:$PORT/v1/chat/completions -H 'Content-Type: application/json' --data @/tmp/qt.json > campt07/${1}_p${i}.json 2>/dev/null; done; }
run_cfg(){ pkill -9 -f llama-server 2>/dev/null; sleep 2
  if [ -z "$3" ]; then CUDA_VISIBLE_DEVICES=0,1 $SRV -m "$2" -ngl 99 -c 4096 --port $PORT --host 127.0.0.1 -fa on --jinja --spec-type none > campt07/srv_$1.log 2>&1 &
  else CUDA_VISIBLE_DEVICES=0,1 $SRV -m "$2" -md "$3" -ngl 99 -ngld 99 -c 4096 --port $PORT --host 127.0.0.1 -fa on --jinja --spec-type draft-dflash --spec-draft-n-max $4 > campt07/srv_$1.log 2>&1 &
  fi
